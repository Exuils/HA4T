...
1. **读页面索引**：列出 `pom/` 下所有非 `_` 开头的 `.py` 文件，只读每个文件头部注释（`# page:` / `# desc:` / `# triggers:`）。**不读 ELEMENTS 内容**。
2. **选页**：根据用户自然语言描述，从 desc/triggers 判断本流程涉及哪些 page。只把这些 page 文件完整读入 ELEMENTS。
3. **读全局**：读 `pom/_meta.py` 获取全局 `VARS`（包名 / 基础 URL / 测试账号等跨用例常量）。
4. **生成用例**：按下方格式写出 `.py` 文件到 `testcases/`。可自由组合 Python 标准库与 Device API。用例顶部建议声明 `LOCAL_VARS = {...}`。
5. **缺失元素**：若流程需要的任意元素在 ELEMENTS 中不存在（或当前平台分桶为空）→ **禁止编造 locator**。在回复末尾输出缺失清单，每项包含：`page 名 | 建议元素名 | 用途 | 需要平台`，并**引导用户到编辑器采集**：
   - *"请在 HA4T 编辑器中打开 {page 名} 页面，使用左上角手柄的「采集模式」，在屏幕上框选对应元素，保存后通知我重试。"*
   - 若只是某元素缺少当前平台分桶：*"请在编辑器打开 {page 名}，切换到 {平台} 设备后重新采集该元素。"*
...
- 用例是**纯 Python 脚本**。可自由使用 Python 标准库（`time` / `json` / `csv` / `math` / `for` / `while` / `if` / `list` / `dict` / `set`……）与下方 Device API 组合
- **禁止新建、修改或删除 `pom/` 目录下的任何文件**（包括 `.py` 和 `_meta.py`）。POM 只由编辑器采集生成——你无法知道实际页面上的 resourceId、text、xpath 等值，猜了也是错的
- **只能引用 `ELEMENTS` 中已存在的元素**（按 pom 文件里的 key 原样写）
- 唯一可以调整的参数是已有元素的 `index`（列表第几个）：`dev.click(商品页.ELEMENTS["商品项"].format(index=2))`
- 每个 `# --step--` 标记一个逻辑步骤，标记后可以是任意复杂度的 Python 代码（一行 / 循环 / 函数调用……）
- 临时定位（已知的临时元素，如用户明确说的文字）有限允许：`dev.click(Selector(android={"text": "确定"}))` 或 `dev.click(text="确定")`——但**不要把它当成常规手段，不能用它来绕过缺失元素清单**
...
**给用户的引导（缺失清单每项附带）**：
  - *"请在 HA4T 编辑器中打开{page}，使用采集模式框选「{建议元素名}」对应的控件，保存后告诉我重试。"*
  - *"如果 {page} 已在当前平台采集但缺这个元素，可能是页面更新了——重新采集该页面即可。"*
