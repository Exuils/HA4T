...
用例放在工作区的 `testcases/` 目录；运行时工作目录为工作区根，`from pom import` 与 `dev.click(image=...)` 自动解析。
...
4. **生成用例**：按下方"用例文件格式"写出 `.py` 文件，保存到工作区的 `testcases/` 目录。用例文件顶部必须声明 `LOCAL_VARS = {...}` 字典存放本用例特有的常量（如本用例的测试账号、固定文案、时间戳种子），编辑器据此自动渲染出可视化变量编辑器。
...
- 复用已有整个用例：`include("other_case.py")`（被引用文件同在 `testcases/` 目录）。
