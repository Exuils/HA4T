...
4. **生成用例**：按下方"用例文件格式"写出 `.py` 文件，保存到 `testcases/`。用例顶部必须声明 `LOCAL_VARS = {...}` 字典存放本用例特有的常量。
...
from pom import 登录页, HomePage, VARS
...
- 每个步骤一个 `# --step--` 标记 + 一行（或几行）代码；标记后文字是步骤备注。
- 元素一律 `dev.click(Page.ELEMENTS["name"])` 形式，**不要把 locator 字面量复制进用例**。
- Page 名与元素名都允许中文。按 pom 文件里实际 key 原样写，不自行翻译。
- 临时定位（不进 POM）有两种写法，都接受：
  - `dev.click(Selector(android={"text": "确定"}, ios={"label": "OK"}))` 显式 Selector
  - `dev.click(text="确定")` —— canonical kwargs 路径：HA4T 内部按当前平台翻译（`text=` 在 iOS 自动用 `label=`）
- 变量：全局 `VARS["key"]`（pom/_meta.py 维护）/ 本用例特有 `LOCAL_VARS["key"]`（顶层字典）。`LOCAL_VARS` 命名固定。
- 复用整个用例：`include("other_case.py")`（被引用文件同在 `testcases/`）。
- 参数化元素（列表第 N 个等）：`dev.click(商品页.ELEMENTS["商品项"].format(index=2))`。Selector 自带 `.format(**kwargs)`，按平台分桶逐字段 `str.format`。
...
2. 读这三个文件 + _meta.py
3. 检查每个引用的元素当前 platform 是否已采集；缺 `SearchPage.ELEMENTS["search_input"]` 在 ios 平台 → 停下输出缺失清单。
4. 生成 `test_search_hotpot.py`，所有 `dev.click(Page.ELEMENTS["name"])` 不带 `**`。
